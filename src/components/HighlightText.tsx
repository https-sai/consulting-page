...
export default function HighlightText({
  ...
  return (
    <motion.span
      ...
      animate="rest"
    >
      <span className={`relative z-10 text-black ${className}`}>{text}</span>

      <motion.span
        variants={variants}
        transition={{ duration: 0.35, ease: "easeOut" }}
        className="absolute inset-y-0 left-0 z-0 rounded-sm bg-black"
      />
      ...
      >
        <span className={`block text-white ${className}`}>{text}</span>
      </motion.span>
    </motion.span>
  );
}
